...
class PrepareResultsSGACorruptions(PrepareResultsBase):
	...
	def __init__(self):
		...
		self.latex_context_fraction_list = ['0.9']
	...
	def generate_paper_sga_corruptions_latex_table(self, sga_results_json):
		...
		num_rows = 2 * len(self.latex_method_list) * len(self.latex_corruption_types) * len(self.latex_mode_list) * len(self.latex_context_fraction_list)
