...
def transfer_csv_from_file_to_database_sga(result_file_path):
...
def transfer_csv_from_file_to_database_sgg(result_file_path):
	base_file_name = os.path.basename(result_file_path)
	
	details = base_file_name.split('_')
	method_name = details[0]
	mode = details[1]
	severity_level = int(details[-1][:-4])
	dataset_corruption_type = details[2]
	video_corruption_type = details[3]
	corruption_type = "_".join(details[2:])[:-4]
	task_name = const.SGG
	...
	with open(result_file_path, 'r') as read_obj:
		...
		for row in csv_reader:
			...
			print("-----------------------------------------------------------------------------------")
			print("Saving result: ", result.result_id)
			db_service.update_result_to_db("results", result.result_id, result.to_dict())
			print("Saved result: ", result.result_id)
			print("-----------------------------------------------------------------------------------")
	...
	for result_directory in os.listdir(results_parent_directory):
		...
		for file_name in os.listdir(result_directory_path):
			if file_name.endswith(".csv"):
				transfer_csv_from_file_to_database_sgg(os.path.join(result_directory_path, file_name))
