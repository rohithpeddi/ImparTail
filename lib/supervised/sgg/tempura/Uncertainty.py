...
class uncertainty_values:
        ...
        for rel in self.cls_rel_uc.keys():
            for cls in self.cls_rel_uc[rel].keys():
                if len(self.cls_rel_uc[rel][cls]['al']) > 0:
                    self.cls_rel_uc[rel][cls]['al'] = (
                    mean(self.cls_rel_uc[rel][cls]['al']), pstdev(self.cls_rel_uc[rel][cls]['al']))
                    self.cls_rel_uc[rel][cls]['ep'] = (
                    mean(self.cls_rel_uc[rel][cls]['ep']), pstdev(self.cls_rel_uc[rel][cls]['ep']))
    ...
    def stats2(self):
        ...
        for rel in self.cls_rel_uc.keys():
            for cls in self.cls_rel_uc[rel].keys():
                if len(self.cls_rel_uc[rel][cls]['al']) > 0:
                    self.cls_rel_uc[rel][cls]['both'] = sum(
                        np.exp(self.cls_rel_uc[rel][cls]['al'] + self.cls_rel_uc[rel][cls]['al']))
    ...
    for u in weight_type:
        ...
        if obj_unc:
            ...
            for k in unq_batch_classes:
                ...
                batch_unc[index[unq_idx], batch_classes[unq_idx]] = np.exp(
                    batch_unc[index[unq_idx], batch_classes[unq_idx]]) / (cls_obj_uc[cls][u] + 1e-12)
        ...
        for rel in ['attention', 'spatial', 'contacting']:
            ...
            for k in unq_batch_classes:
                ...
                batch_unc[index[unq_idx], batch_classes[unq_idx]] = np.exp(
                    batch_unc[index[unq_idx], batch_classes[unq_idx]]) / (cls_rel_uc[rel][k][u] + 1e-12)
