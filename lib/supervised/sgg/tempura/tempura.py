...
from lib.supervised.sgg.tempura.gmm_heads import *
from lib.supervised.sgg.tempura.transformer import transformer
...
class TEMPURA(nn.Module):
    ...
    def __init__(self, mode='sgdet', attention_class_num=None, spatial_class_num=None,
        ...
        self.object_classifier = ObjectClassifier(mode=self.mode, obj_classes=self.obj_classes,
                                                  obj_head=obj_head, mem_compute=obj_mem_compute, K=K,
                                                  selection=selection,
        ...
        if rel_head == 'gmm':
            self.a_rel_compress = GMM_head(hid_dim=1936, num_classes=self.attention_class_num, rel_type='attention',
                                           k=self.GMM_K)
            self.s_rel_compress = GMM_head(hid_dim=1936, num_classes=self.spatial_class_num, rel_type='spatial',
                                           k=self.GMM_K)
            self.c_rel_compress = GMM_head(hid_dim=1936, num_classes=self.contact_class_num, rel_type='contact',
                                           k=self.GMM_K)
