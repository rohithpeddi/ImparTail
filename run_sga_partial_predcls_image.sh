...
generate_run_scripts() {
    ...
    for task in "${TASKS[@]}"; do
        for method_name in "${METHODS[@]}"; do
            for partial_percentage in "${PARTIAL_PERCENTAGES[@]}"; do
                        ...
                        if [[ "$dataset_corruption_mode" == "fixed" ]]; then
                            for severity_level in "${SEVERITY_LEVELS[@]}"; do
                                for corruption_type in "${CORRUPTION_TYPES[@]}"; do
                                    ...
                                    python test_sga_methods.py --task_name $task --method_name $method_name --ckpt $CKPT_DIRECTORY_PATH/${task}/${method_name}/${method_name}_partial_${partial_percentage}_${mode}_future_3_epoch_3.tar --use_input_corruptions --dataset_corruption_mode $dataset_corruption_mode --dataset_corruption_type $corruption_type --corruption_severity_level $severity_level
                                    echo "-----------------------------------------------------------------------------"
                                done
                            done
                            ...
                            for severity_level in "${SEVERITY_LEVELS[@]}"; do
                                for video_corruption_mode in "${VIDEO_CORRUPTION_MODES[@]}"; do
                                    # Execute Python script directly for each combination
                                    python test_sga_methods.py --task_name $task --method_name $method_name --ckpt $CKPT_DIRECTORY_PATH/${task}/${method_name}/${method_name}_partial_${partial_percentage}_${mode}_future_3_epoch_3.tar --use_input_corruptions --dataset_corruption_mode $dataset_corruption_mode --video_corruption_mode $video_corruption_mode --dataset_corruption_type $video_corruption_mode --corruption_severity_level $severity_level
                                    echo "-----------------------------------------------------------------------------"
                                done
                            done
                        fi
