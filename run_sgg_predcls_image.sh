                    ...
                    if [[ "$dataset_corruption_mode" == "fixed" ]]; then
                        for severity_level in "${SEVERITY_LEVELS[@]}"; do
                            for corruption_type in "${CORRUPTION_TYPES[@]}"; do
                                ...
                                python test_sgg_methods.py --use_wandb --task_name $task --method_name $method_name --ckpt $CKPT_DIRECTORY_PATH/${task}/${method_name}/${method_name}_${mode}_epoch_3.tar --use_input_corruptions --dataset_corruption_mode $dataset_corruption_mode --dataset_corruption_type $corruption_type --corruption_severity_level $severity_level
                                echo "-----------------------------------------------------------------------------"
                            done
                        done
