...
class TestSGGBase(STSGBase):
    ...
    def _write_evaluation_statistics(self):
        ...
        if self._conf.use_input_corruptions:
            ...
            file_name = f'{self._conf.method_name}_partial_{self._conf.partial_percentage}_{self._conf.mode}.csv'
        elif self._conf.use_label_noise:
            scenario_dir = os.path.join(task_dir, "labelnoise")
            file_name = f'{self._conf.method_name}_label_noise_{self._conf.label_noise_percentage}_{self._conf.mode}.csv'
        else:
